...
let accumulator = 0;
// Both techniques are toggleable at runtime so their effect can be felt.
...
love.draw = () => {
  ...
  love.graphics.print(`[i] interp ${interpolation ? 'on' : 'off'}`, 3, GAME_HEIGHT - 19);
  love.graphics.setColor(1, 1, 1, subpixel ? 1 : 0.45);
  love.graphics.print(`[p] subpix ${subpixel ? 'on' : 'off'}`, 3, GAME_HEIGHT - 11);
